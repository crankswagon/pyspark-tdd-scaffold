...
def setup_bronze_db(spark, bronze_context: BronzeContext):
    spark.sql(f'CREATE DATABASE IF NOT EXISTS {bronze_context.destination_database_name}')
    
    yield
    
    spark.sql(f'DROP TABLE IF EXISTS {bronze_context.destination_database_name}.{bronze_context.destination_table_name}')
    spark.sql(f'DROP DATABASE IF EXISTS {bronze_context.destination_database_name}')
...
def test_stream_load(spark, bronze_context: BronzeContext, infer_schema):
    ...
    bronze_output = spark.sql(f'select * from {bronze_context.destination_database_name}.{bronze_context.destination_table_name}')
